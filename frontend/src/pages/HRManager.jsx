...
function HRManager() {
  ...
  const [newEmployee, setNewEmployee] = useState({
    full_name: '', phone: '', position: '', role_code: 'cashier', 
    salary: '', status: 'Active', username: '', password: '', permissions: []
  });

  const fetchEmployees = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/employees`);
      // 🚀 መረጃው ትክክለኛ Array መሆኑን አረጋግጦ ያስገባል (ገጹ እንዳይጠፋ ይከላከላል)
      ...
      setEmployees([]); // ኤረር ሲመጣም ባዶ አሬይ አድርጎ ያድነዋል
    }
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === 'position') {
      ...
      else if (value.includes('Cashier')) defaultPerms = ['pos']; 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSubmit = {
        ...newEmployee,
        permissions: newEmployee.permissions.join(',') 
      };
  ...
  const openEditModal = (emp) => {
    ...
    // 🚀 ፈቃዶችን ያለምንም ክራሽ በሰላም ለመክፈል
    const permsList = Array.isArray(emp.permissions) 
      ? emp.permissions 
      : typeof emp.permissions === 'string' && emp.permissions.trim() !== '' 
        ? emp.permissions.split(',') 
        : [];
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-100 border-b">
                ...
                <th className="p-3 text-slate-700 font-bold text-sm">Permissions</th>
            ...
            <tbody>
              {!Array.isArray(employees) || employees.length === 0 ? (
                ...
                employees.map((emp) => {
                  // 🚀 የፈቃድ መረጃው Array መሆኑን በማረጋገጥ የገጹን ሞት ይከላከላል
                  const permsList = Array.isArray(emp.permissions) 
                    ? emp.permissions 
                    : typeof emp.permissions === 'string' && emp.permissions.trim() !== '' 
                      ? emp.permissions.split(',') 
